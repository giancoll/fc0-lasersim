...
void delta(std::string filename, int med, float num) {
    TFile *file = TFile::Open(("dati_matteo/"+filename+".root").c_str());
    ...
    TFile *newfile = TFile::Open(("filt/"+filename+".root").c_str(), "RECREATE");
    newfile->cd();
    TTree *newtree = new TTree("anode", "Tree con punti filtrati in Y");

    //std::ofstream outfile("val_matrici.txt");
    
    int medpes = med; // 0 media per righe, 1 media per colonne
    float iniz = 2; 
    float val = -num;
    float vd = 0.0801; 
    float val2 = 187/vd;
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        ...
        for (int j = 0; j < n_elements; j++) {
		        ...
		        float dist1 = valuet->at(j)-val2; float dist2;
		        if (medpes==1){
