...
ND::TUnitsTable*
ND::TUnitsTable::fTUnitsTable = NULL;

ND::TUnitsTable::TUnitsTable() {
...
std::string ND::TUnitsTable::ConvertWithUnit(std::string input) {
    ...
    if (units.find(unit) == units.end()) {
        PrintListOfUnits();
        ND280Error("ND::TUnitsTable: requested unit '"
                   << unit << "' not found.");
        return "badUnit";
    }
...
std::string ND::TUnitsTable::ConvertEnergy(double enr) {
...
std::string ND::TUnitsTable::ConvertLength(double len) {
    std::stringstream output;
...
std::string ND::TUnitsTable::ConvertCharge(double q) {
    std::stringstream output;
...
std::string ND::TUnitsTable::ConvertTime(double tim) {
    std::stringstream output;
    double val;
...
void ND::TUnitsTable::PrintListOfUnits() {
    std::cout << std::endl;
...
std::string unit::AsString(double val, double sig, std::string type) {
    ND::TUnitsTable& u = ND::TUnitsTable::Get();
    std::ostringstream measure;
    if (type == "length") {
        measure << u.ConvertLength(val);
        if (sig >= 0) {
            if (ND::TCorrValues::IsFree(sig)) measure << " (free)";
            else if (ND::TCorrValues::IsFixed(sig)) measure << " (fixed)";
        ...
        else if (ND::TCorrValues::IsFree(sig)) measure << " deg (free)";
        else if (ND::TCorrValues::IsFixed(sig)) measure << " deg (fixed)";
        ...
        measure << val;
        if (sig<0) measure << " e";
        else if (ND::TCorrValues::IsFree(sig)) measure << " e (free)";
        else if (ND::TCorrValues::IsFixed(sig)) measure << " e (fixed)";
        else measure << "+-" << sig << " e";
    }
    else if (type != "") {
        measure << std::fixed;
        ...
        else if (ND::TCorrValues::IsFree(sig)) {
            measure << " <" << type << "> (free)";
        }
        else if (ND::TCorrValues::IsFixed(sig)) {
            measure << " <" << type << "> (fixed)";
        }
