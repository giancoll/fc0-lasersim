#ifndef THATPadManager_hxx
#define THATPadManager_hxx
...
namespace ND {
    class THATPadManager;
}

/// Provide low level interfaces to the HAT pad geometry data base.  The
/// HATPadManager interface is used by oaGeomInfo which provides a higher
/// level (more friendly) interface.  The low level interface is primarily
/// used to store the HAT pad geometry inside of the event stream, or to
/// interface to external data bases.
/// A ERAM/MM module corresponds to one FEM and is composed of two FEC 
/// which are composed of 8 AFTER chips each.
/// Each After maps an area of 9x8 pads (72 pads) -> 36 x 32 pads per ERAM/MM.
/// Each pad is then mapped as 
/// 1  ..  .. 32
/// 33 ..  .. 64
/// .. ..  .. ..
///
/// This manager provides for one ERAM/MM module:
...
///
class ND::THATPadManager: public TNamed {
public:
    THATPadManager();
    virtual ~THATPadManager();
...
private:
    ...
    THATPadManager(const THATPadManager&) {MayNotUse("Copy Constructor");}
