...
namespace ND {
    ...
    /// Cannot find requested DigitFactory
    OA_EXCEPTION(EDigitFactNotFound, EDigitManager);
...
/// using oaRawEvent.  The eventUnpack library will implement a digit factory
...
/// oaRawEvent (using eventUnpack).  This class will not be directly used by most
...
public: 
    ...
    /// fill the cache of digits.  The will be registered by eventUnpack, and
    ...
    /// Get a reference to the TDigitFactory, if it exists.
    /// If there is no TDigitFactory with the name, throws an exception.
    ND::TDigitFactory& GetFactory(std::string type);
