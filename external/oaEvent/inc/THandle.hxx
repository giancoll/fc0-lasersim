...
namespace ND {
    ...
    ///
    /// \code
    /// ND::THandle<ND::THit> aHandle(aHitFromSomePlace);
    /// aHandle.IsWeak()     // Returns false.
    ///
    /// ND::THandle<ND::THit> bHandle = aHandle;
    ...
    public:
        ...
        void IncrementHandleCount() {
            ...
            if (fHandleCount > 30000) {
                ND280Error("To many handles for object: " << fHandleCount);
            }
        }
...
//////////////////////////////////////////////////////////////////
template <class T>
ND::THandle<T>::THandle(T* pointee) {
...
template <class T>
ND::THandle<T>::THandle() {
    Default(NULL);
}

template <class T>
ND::THandle<T>::THandle(T* pointee, bool owner) {
    if (pointee) {
        if (owner) 
...
template <class T>
ND::THandle<T>::THandle(const THandle<T>& rhs) : TVHandle(rhs) {
    Default(NULL);
    Link(rhs);
...
template <class T>
template <class U> 
ND::THandle<T>::THandle(const ND::THandle<U>& rhs) {
    Default(NULL);
...
template <class T>
ND::THandle<T>& ND::THandle<T>::operator = (THandle<T>& rhs) {
...
template <class T> 
const ND::THandle<T>& ND::THandle<T>::operator = (const THandle<T>& rhs) {
    // Going to replace the value of this smart pointer, so unref and
    // possible delete.
    Unlink();
    // Compatible types
...
template <class T> 
template <class U>
ND::THandle<U>& ND::THandle<T>::operator = (THandle<U>& rhs) {
    // Going to replace the value of this smart pointer, so unref and
    // possible delete.
    Unlink();
    // Compatible types
...
template <class T> 
template <class U>
const ND::THandle<U>& ND::THandle<T>::operator = (const THandle<U>& rhs) {
    // Going to replace the value of this smart pointer, so unref and
    // possible delete.
    Unlink();
    // Compatible types
